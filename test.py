import pygame
from pygame.constants import (K_SPACE, K_ESCAPE, K_a, K_d, K_w, K_s, K_LEFT, K_RIGHT, K_UP, K_DOWN, K_m, K_k)
...
# ====================== 资源加载 ======================
# 背景图片
bg0 = pygame.image.load("start_bg0.jpg")  # 开始界面背景
bg1 = pygame.image.load("map1.jpg")  # 游戏主背景
start = pygame.image.load("start.png")  # 开始按钮
...
# 字体
myFont = pygame.font.SysFont("simhei", 45)  # 大标题字体
info_font = pygame.font.SysFont("simhei", 25)  # 信息提示字体
score_font = pygame.font.SysFont("simhei", 36)  # 分数显示字体
...
# ====================== 最高分系统 ======================
HIGH_SCORE_FILE = "high_score.json"  # 最高分存储文件
...
# ====================== 游戏函数 ======================
def check_collision(rect1: Union[pygame.Rect, Tuple[int, int, int, int]],
                    rect2: Union[pygame.Rect, Tuple[int, int, int, int]],
                    radius1: Optional[float] = None,
                    radius2: Optional[float] = None) -> bool:
    """
    基于中心点的圆形碰撞检测

    参数:
        rect1: 第一个矩形对象或(x,y,w,h)元组
        rect2: 第二个矩形对象或(x,y,w,h)元组
        radius1: 第一个对象的碰撞半径(可选)
        radius2: 第二个对象的碰撞半径(可选)
...
def show_start_screen() -> Optional[bool]:
    ...
    返回:
        Optional[bool]: True表示选择鼠标控制，False表示键盘控制，None表示退出
    """
    screen.blit(bg0, (0, 0))
    ...
    running = True
    selected_control = None  # 用于存储选择的控制方式
    while running:
...
def run_game(mouse_control: bool) -> None:
    ...
    # ========== 连击系统 ==========
    combo_window = 0.5  # 连击时间窗口(秒)
    hit_times = []  # 记录击中时间戳
    combo_count = 0  # 连击计数
    last_combo_score_time = 0  # 上次获得连击分数的时间
    ...
    while running and not game_over:
        ...
        # ========== 处理连击系统 ==========
        # 清理过期的击中记录(超过combo_window时间的)
        while hit_times and current_time - hit_times[0] > combo_window:
            hit_times.pop(0)

        # 检查是否达成连击(至少2次击中在时间窗口内)
        if len(hit_times) >= 2:
            # 确保两次击中不是同一物体(通过时间差判断)
...
if __name__ == '__main__':
    while True:
        # 获取选择的控制方式（True为鼠标控制，False为键盘控制）
